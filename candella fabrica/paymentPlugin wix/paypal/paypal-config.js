...
export const createTransaction = async (options) => {
    ...
    try {
        ...
        const subtotalInCents = itemsTotal + (shippingCost || 0);

        // PayPal Transaction Fee: 2.9% + $0.30 AUD
        const paypalFeePercentage = 0.029; // 2.9%
        const paypalFixedFee = 30; // $0.30 in cents
        const paypalFee = Math.ceil((subtotalInCents * paypalFeePercentage) + paypalFixedFee);

        await logToCMS('createTransaction', 'INFO', 'PayPal fee calculated', {
            itemsTotal,
            shippingCost,
            subtotalInCents,
            paypalFee,
            grandTotal: subtotalInCents + paypalFee
        });
